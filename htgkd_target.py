...
def obtain_label_nh(feas, label_old, Kf):
    ...
    for k in range(num_nn_max):
        ...
        if k > 0:
            ent_mean, ent_vct = get_ent(oh_final)
            log_ent_mean.append(ent_mean.item())
            log_ent_vct.append(ent_vct)
    
    ent_diff = [log_ent_mean[i] - log_ent_mean[i+1] for i in range(len(log_ent_mean)-1)]
    ent_gap = np.array(ent_diff)
    ent_gap_normed = ent_gap / max(ent_gap)
    idx_attention = np.where(ent_gap_normed > 0.99)[0] + 3
    num_nn = 5
